...
function recerberMelhorEstudante(obj){
    ...
    let nomesAlunos = Object.keys(obj)
    let notasAlunos = Object.values(obj)
    let mediasAlunos = []
 

    notasAlunos.forEach(element => {
        notaTotal = element.reduce((acc,valorAtual) => acc + valorAtual)  
        mediasAlunos[notasAlunos.indexOf(element)] = notaTotal / element.length
    });

    console.log(mediasAlunos)

    melhorNota = mediasAlunos.reduce((acc,valorAtual) => valorAtual > acc ? valorAtual : acc )
    melhorAluno['nome'] = nomesAlunos[mediasAlunos.indexOf(melhorNota)]
    melhorAluno['media'] = melhorNota
